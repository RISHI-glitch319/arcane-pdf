...
import {
  FileImage, Download, Activity, RefreshCcw, ArrowLeft,
  Sparkles, Moon, Sun, Info, Loader2
} from "lucide-react";
  ...
  const handleUpload = () => {
    ...
    xhr.onload = () => {
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        alert("Rasterization failed: Matrix sequence interrupted.");
        setLoading(false);
      }
    };

    xhr.open("POST", `${API_BASE}/pdf-to-jpg`);
        ...
        >
          ...
          <header className="text-center mb-12">
            ...
            <h1 className={`text-4xl font-black mb-2 uppercase italic tracking-tighter ${dark ? 'text-white' : 'text-[#050412]'}`}>
              PDF to <span style={{ color: BRAND.magenta }}>JPG</span>
            </h1>
            <p className="text-slate-500 font-bold uppercase text-[10px] tracking-[0.2em]">Visual Asset Extraction Engine • 2026</p>
          </header>
            ...
            {!downloadUrl && !loading ? (
              <motion.div key="input" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-8">
                <PdfUploadDropzone
                  ...
                  helperText={file ? `Vector stream ready • ${formatPreviewSize(file.size)}` : "Protocol: Awaiting Vector Stream"}
                  icon={<FileImage style={{ color: BRAND.magenta }} size={56} />}
                />
                ...
                <LivePreviewCard dark={dark} title="PDF Source Preview" caption="Input">
                  <div className="space-y-4">
                    ...
                    <PdfPreview url={previewUrl} title="PDF to JPG input preview" dark={dark} emptyLabel="Selected PDF preview appears here before rasterization." />
                  </div>
                </LivePreviewCard>
                ...
                <button
                  ...
                  disabled={!file}
                  style={{ backgroundColor: !file ? 'transparent' : BRAND.magenta }}
                  className={`w-full py-6 rounded-[2rem] font-black text-xl transition-all shadow-xl active:scale-[0.98] disabled:opacity-20 uppercase italic ${!file ? 'bg-white/5 border border-white/5 text-slate-600' : 'text-white hover:brightness-110 shadow-[#CC208E]/20'}`}
                >
              ...
              <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="py-20 text-center space-y-6">
                ...
                <div>
                  ...
                  <div className={`mt-4 h-1 w-48 mx-auto rounded-full overflow-hidden ${dark ? 'bg-white/10' : 'bg-slate-100'}`}>
                    <motion.div
                      className="h-full bg-[#CC208E]"
                      initial={{ width: 0 }}
                      animate={{ width: `${uploadProgress}%` }}
                    />
                  </div>
                  <p className="text-slate-500 text-[10px] mt-2 font-bold uppercase">Deconstructing Asset Layers</p>
                </div>
              </motion.div>
            ) : (
              <motion.div key="result" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center space-y-10">
                <LivePreviewCard dark={dark} title="Image Output Preview" caption="Output">
                  <div className={`rounded-[1.75rem] border p-6 text-left ${dark ? 'border-white/10 bg-black/20' : 'border-slate-200 bg-white'}`}>
                    <p className={`text-sm font-black uppercase tracking-[0.25em] ${dark ? 'text-white' : 'text-slate-900'}`}>ZIP archive ready</p>
                    <p className={`mt-3 text-xs font-bold uppercase tracking-[0.2em] ${dark ? 'text-slate-400' : 'text-slate-500'}`}>
                      {file ? `Rasterized assets from ${file.name} are prepared for download.` : "Your extracted image archive is ready."}
                    </p>
                  </div>
                </LivePreviewCard>
                ...
                <div className={`p-12 rounded-[3rem] border relative overflow-hidden ${dark ? 'bg-[#CC208E]/10 border-[#CC208E]/20' : 'bg-slate-50 border-slate-100'}`}>
                  ...
                  <p style={{ color: BRAND.magenta }} className="text-[10px] font-black tracking-widest uppercase mt-2">ZIP Archive Synthesized</p>
                ...
                <div className="space-y-4">
                  <a
                    href={downloadUrl!}
                    download={`arcane_assets_${file?.name.replace('.pdf', '')}.zip`}
                  ...
                  >
                    <Download size={24} /> Extract Visual Assets
                  </a>
                  <button
