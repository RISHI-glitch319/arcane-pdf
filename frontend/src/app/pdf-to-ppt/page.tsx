...
import { ArrowLeft, Loader2, FileType, CheckCircle2, LayoutTemplate, RefreshCcw, Moon, Sun, Info, Sparkles, Activity } from "lucide-react";
...
/* ================= PDF TO PPT COMPONENT ================= */
export default function PDFToPPT() {
  // 1. LOCAL STATE ONLY: No localStorage persistence or initial retrieval
  ...
  // 2. STANDARDIZE TOGGLE: Removed all storage.setItem calls
  const toggleTheme = () => {
    setDark(prev => !prev);
  };

  // Removed useEffect that checked for "arcane-theme" in localStorage
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/pdf-to-ppt`, {
        method: "POST",
        body: formData,
      });
      ...
      alert("Sovereign Engine Error: PPT rasterization sequence interrupted.");
      clearInterval(interval);
    } finally {
        ...
        >
          ...
          <header className="text-center mb-10">
            ...
            <h1 className={`text-4xl font-black mb-3 uppercase italic tracking-tighter ${dark ? 'text-white' : 'text-[#050412]'}`}>
              PDF to <span style={{ color: BRAND.magenta }}>PowerPoint</span>
            </h1>
            <p className="text-slate-500 font-bold uppercase text-[10px] tracking-[0.2em]">Protocol: Turn Static PDF into Presentation Assets • 2026</p>
          </header>
            ...
            {!resultUrl ? (
              <motion.div key="input" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-8">
                <PdfUploadDropzone
                  ...
                  helperText={file ? `Sequence detected • ${formatPreviewSize(file.size)}` : "Protocol: Awaiting Presentation Data"}
                  icon={<FileType style={{ color: BRAND.magenta }} size={56} />}
                />
                ...
                <LivePreviewCard dark={dark} title="PDF Source Preview" caption="Input">
                  <div className="space-y-4">
                    ...
                    <PdfPreview url={previewUrl} title="PDF to PowerPoint input preview" dark={dark} emptyLabel="Selected PDF preview appears here before conversion." />
                  </div>
                </LivePreviewCard>
              ...
              <motion.div key="result" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center space-y-10">
                <LivePreviewCard dark={dark} title="PowerPoint Output Preview" caption="Output">
                  <div className={`rounded-[1.75rem] border p-6 text-left ${dark ? 'border-white/10 bg-black/20' : 'border-slate-200 bg-white'}`}>
                    <p className={`text-sm font-black uppercase tracking-[0.25em] ${dark ? 'text-white' : 'text-slate-900'}`}>Presentation package ready</p>
                    <p className={`mt-3 text-xs font-bold uppercase tracking-[0.2em] ${dark ? 'text-slate-400' : 'text-slate-500'}`}>
                      {file ? `${file.name.replace(".pdf", ".pptx")} prepared for download.` : "Your presentation export is ready."}
                    </p>
                  </div>
                </LivePreviewCard>
                ...
                <div className={`p-8 rounded-[3rem] border flex items-center gap-6 text-left ${dark ? 'bg-[#CC208E]/10 border-[#CC208E]/20' : 'bg-slate-50 border-slate-200'}`}>
                  ...
                  <div>
                    <p className={`font-black text-2xl italic leading-tight uppercase ${dark ? 'text-white' : 'text-slate-900'}`}>Presentation Ready!</p>
                ...
                <div className="space-y-4">
                  ...
                  >
                    <RefreshCcw size={14} /> Start New Conversion
                  </button>
                </div>
              </motion.div>
            )}
