...
import { ArrowLeft, Loader2, Presentation, CheckCircle2, RefreshCcw, Moon, Sun, Sparkles, Download } from "lucide-react";
...
/* ================= MAIN PPT TO PDF PAGE ================= */
export default function PPTToPDF() {
  // 1. LOCAL STATE ONLY - No restoration from shared storage
  ...
  // 2. STANDARDIZE TOGGLE - No side effects to external storage
  const toggleTheme = () => {
    setDark(prev => !prev);
  };

  // 3. HANDLE FILE PREVIEW GENERATION (API Logic Preserved)
  useEffect(() => {
  ...
  // 4. MEMORY SAFETY - Isolated Cleanup
  useEffect(() => {
    return () => {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/ppt-to-pdf`, {
        method: "POST",
        body: formData,
      });
      ...
      alert("Error: Ensure Microsoft PowerPoint is installed on the server.");
      clearInterval(interval);
    } finally {
        ...
        <div className="flex justify-between items-center max-w-7xl mx-auto px-6 md:px-12 py-4">
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <span className={`text-xl font-black tracking-tighter uppercase italic transition-colors ${dark ? 'text-white' : 'text-[#050412]'}`}>
              Arcane <span style={{ color: BRAND.magenta }}>Presenter</span>
            </span>
          </Link>
        ...
        >
          ...
          <header className="text-center mb-10">
            ...
            <h1 className={`text-4xl font-black mb-3 uppercase italic tracking-tighter ${dark ? 'text-white' : 'text-[#050412]'}`}>
              PPT to <span style={{ color: BRAND.magenta }}>PDF</span>
            </h1>
            <p className="text-slate-500 font-bold uppercase text-[10px] tracking-[0.2em]">Protocol: Slide-to-Document Distillation • 2026</p>
          </header>
            ...
            {!resultUrl ? (
              <motion.div key="input" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-8">
                <div className={`group relative border-2 border-dashed rounded-[2.5rem] p-16 text-center transition-all cursor-pointer ${dark ? 'border-white/10 hover:border-[#CC208E]/40 hover:bg-[#CC208E]/5' : 'border-slate-200 hover:border-[#CC208E]/40 hover:bg-slate-50'}`}>
                  ...
                  <div className="relative z-10">
                    <Presentation style={{ color: BRAND.magenta }} className="mx-auto mb-6 group-hover:scale-110 group-hover:rotate-6 transition-transform duration-500" size={56} />
                    <p className={`font-black text-xl uppercase italic tracking-tight ${dark ? 'text-white' : 'text-slate-900'}`}>
                      {file ? file.name : "Inject PPT Sequence"}
                    </p>
                    <p className="text-slate-500 mt-2 font-bold uppercase text-[10px] tracking-[0.2em]">Protocol: Awaiting Presentation Matrix</p>
                  </div>
                </div>

                <LivePreviewCard dark={dark} title="Presentation Source" caption="Input">
                  <div className="space-y-4">
                    <div className={`grid gap-3 rounded-[1.5rem] border px-4 py-3 text-xs font-bold uppercase tracking-[0.2em] md:grid-cols-2 ${dark ? 'border-white/10 bg-black/20 text-slate-400' : 'border-slate-200 bg-white text-slate-500'}`}>
                      <span>{file ? file.name : "No PPT file selected yet"}</span>
                    ...
                    {previewUrl ? (
                      ...
                      <div className="flex flex-col items-center justify-center min-h-[260px] rounded-[1.75rem] border border-dashed border-white/10 animate-pulse">
                      ...
                      <PdfPreview url={null} title="Presentation source preview" dark={dark} emptyLabel="Uploaded presentation preview appears here." />
                    )}
                  </div>
                </LivePreviewCard>
              ...
              <motion.div key="result" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center space-y-8">
                <LivePreviewCard dark={dark} title="Generated PDF" caption="Output">
                  <PdfPreview url={resultUrl} title="PPT to PDF output preview" dark={dark} emptyLabel="Generated PDF preview appears here." />
                </LivePreviewCard>
                <div className={`p-10 rounded-[3rem] border relative overflow-hidden ${dark ? 'bg-[#CC208E]/10 border-[#CC208E]/20' : 'bg-slate-50 border-slate-100'}`}>
                  ...
                  <p className={`text-2xl font-black uppercase italic ${dark ? 'text-white' : 'text-slate-900'}`}>Presentation Ready!</p>
                  <div style={{ color: BRAND.magenta, backgroundColor: `${BRAND.magenta}20` }} className="mt-2 px-4 py-1 rounded-full text-[10px] font-black tracking-[0.3em] inline-block uppercase">Protocol Serialized</div>
